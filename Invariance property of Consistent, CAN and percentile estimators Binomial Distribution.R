...
for (i in 1:length(n))
{
  ...
  p_hat=x_bar/m;
  p1[i]=mean(abs((1-p_hat)^m-(1-p)^m)<eps);
...
for (i in 1:length(n))
{
  ...
  x_bar=apply(x,2,mean)
  p_hat=x_bar/m;
  T=m*p_hat*(1-p_hat)^(m-1)
  p1[i]=mean(abs(T-m*p*(1-p)^(m-1))<eps);
...
for (i in 1:length(n))
{
  ...
  x_bar=apply(x,2,mean)
  p_hat=x_bar/m;
  T=m*p_hat^2*(1-p_hat)^(m-2)
  p1[i]=mean(abs(T-m*p^2*(1-p)^(m-2))<eps);
